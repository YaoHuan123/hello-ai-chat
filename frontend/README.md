...
2. **前端**：配置 `.env.android`（见上），执行 `npm run build:android:debug` 与 `npm run install:android:debug`。
3. **App 内流程**：手机号 → **获取验证码** → **登录** → 主导航 → **注销账号** → 再次获取验证码 → **确认注销** → 回到登录页。
...
Android 构建使用 **`.env.android`**（`vite build --mode android` 自动加载），不要用 Web 的 `.env`。
...
| `npm run android:lan-ip` | 列出本机局域网 IP，用于填写 `.env.android` |
| `npm run sync:android` | Android 构建并 `cap sync` |
| `npm run run:android` | 同步后在已连接真机/设备上运行 |
| `npm run build:android:debug` | 打 Debug APK |
