import type { MolPrimaryCategory } from "./molWorld";
...
type PersonaPreset = {
  summary: string;
  infoItems: Omit<MolPersonaInfoItem, "id">[];
};

function items(rows: Omit<MolPersonaInfoItem, "id">[]): Omit<MolPersonaInfoItem, "id">[] {
  return rows;
}

const BY_CATEGORY: Record<MolPrimaryCategory, PersonaPreset> = {
  职场沟通: {
...
const BY_MOL_ID: Record<string, Partial<PersonaPreset>> = {
  "mw-seed-pro": {
    infoItems: items([
...
function isValidCategory(v: string): v is MolPrimaryCategory {
  return v in BY_CATEGORY;
}

export function defaultSummaryForCategory(category: string): string {
  if (isValidCategory(category)) return BY_CATEGORY[category].summary;
  return BY_CATEGORY["职场沟通"].summary;
}

export function defaultInfoItemsForCategory(category: string, now = Date.now()): MolPersonaInfoItem[] {
  const preset = isValidCategory(category) ? BY_CATEGORY[category] : BY_CATEGORY["职场沟通"];
  return preset.infoItems.map((row, i) => ({
    id: `inf-def-${now}-${i}`,
    ...row,
  }));
}

export function defaultInfoItemsForMol(molId: string, category: string, now = Date.now()): MolPersonaInfoItem[] {
  const base = isValidCategory(category) ? BY_CATEGORY[category] : BY_CATEGORY["职场沟通"];
  const override = BY_MOL_ID[molId];
  const rows = override?.infoItems ?? base.infoItems;
...
export function defaultSummaryForMol(molId: string, category: string): string {
  ...
  return defaultSummaryForCategory(category);
}
...
export function needsPersonaBackfill(summary: string, infoItems: MolPersonaInfoItem[]): boolean {
  ...
  return !t || t === PLACEHOLDER_MOL_SUMMARY;
}
...
>(record: T, now = Date.now()): T {
  ...
  if (!t || t === PLACEHOLDER_MOL_SUMMARY) {
