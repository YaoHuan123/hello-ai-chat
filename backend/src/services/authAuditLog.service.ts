...
export type AuthAuditEvent = "sms_send" | "sms_login_success" | "sms_login_failed" | "delete_account";
...
export class AuthAuditLogService {
  ...
  record(entry: AuthAuditRecord): void {
    ...
    if (Math.random() < 0.01) {
      this.db.prepare("DELETE FROM audit_auth_log WHERE ts < ?").run(now - RETENTION_DAYS * ONE_DAY_MS);
    }
  }
}
